...
def obtener_conexion():
    ...
    conn.execute("PRAGMA foreign_keys = ON;")  # Activa validación de Claves Foráneas
    return conn
...
def inicializar_bd():
    ...
    # 3. Actualización de autoincrementos para evitar errores de duplicidad en POST
    tablas = ["roles", "usuarios", "especies", "refugios", "animales"]
    for tabla in tablas:
        cursor.execute(f"UPDATE sqlite_sequence SET seq = (SELECT MAX(id) FROM {tabla}) WHERE name = '{tabla}'")
